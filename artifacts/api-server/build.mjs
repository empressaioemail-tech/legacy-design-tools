...
async function buildAll() {
  ...
  await esbuild({
    // Entry points: the server itself and the IFC parse worker (QA-16).
    // esbuild's outbase is their common ancestor `src/`, so they land at
    // `dist/index.mjs` and `dist/lib/ifcParser/ifcParseWorker.mjs`
    // respectively — the paths the respective `workerClient.ts` files resolve
    // at runtime.
    entryPoints: [
