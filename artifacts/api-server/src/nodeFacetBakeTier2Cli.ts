 ...
 *
 * === FEMA-ONLY BY DEFAULT (road leg deferred) ===
 * This bake ships the FEMA flood facet ONLY. The OSM road-envelope upgrade is
 * GATED OFF by default because the shared public Overpass is 504-saturated
 * (PR #319's dry-run measured ~0.1 nodes/sec / ~100% failures) — a per-node
 * county bake against it is not feasible. The road leg is deferred to the
 * private/self-hosted Overpass infra deliverable
 * (doc_repo/_inbox/2026-07-21_overpass_road_data_spec.md). Pass `--enable-roads`
 * (and point OVERPASS_URL at a private mirror) ONLY once that lands. With roads
 * gated off, every node carries the Tier-1-grade point/shape envelope (honestly
 * labeled, never a fabricated road front) plus the real FEMA flood facet.
 *
 * Usage (from repo root):
 *   pnpm --filter @artifacts/api-server node-facet-bake-tier2 -- \
 *     --county=48055 [--limit=200] [--dry-run] [--page-size=2000] \
 *     [--fema-concurrency=4] [--enable-roads --road-concurrency=2] \
 *     [--skip-fema] [--adapter-key=node-facets:tier2]
 *
 *   or directly (FEMA-only dry-run):
...
// Re-exported from the pure constants module so the WRITE key here and the READ
// key in brokerageNodeFacets.ts can never drift (both import the same source).
export { TIER2_ADAPTER_KEY };
...
async function main(): Promise<void> {
  ...
  const { values } = parseArgs({
    ...
    options: {
      ...
      // Roads are GATED OFF by default (FEMA-only ship). The OSM road-envelope
      // leg is INFEASIBLE on the shared public Overpass (504-saturated,
      // measured 0.1 nodes/sec / 100% failures in PR #319's dry-run) and is
      // deferred to the private/self-hosted Overpass infra deliverable
      // (doc_repo/_inbox/2026-07-21_overpass_road_data_spec.md). `--enable-roads`
      // is the explicit opt-in (only sane once OVERPASS_URL points at a private
      // mirror); `--skip-roads` remains as an explicit no-op alias. `--skip-fema`
      // is retained so a road-only run is expressible once roads are enabled, but
      // the DEFAULT bake is FEMA-only.
      "enable-roads": { type: "boolean", default: false },
  ...
  const roadTimeoutMs =
    values["road-timeout-ms"] !== undefined
      ...
      : 12_000;
  // Roads gated off unless EXPLICITLY enabled. `--enable-roads` opts in;
  // `--skip-roads` is a redundant-but-honored no-op (roads already off). This is
  // the FEMA-only ship: the road leg needs a private Overpass (see the block on
  ...
  try {
    ...
    log(
      `${dryRun ? "DRY-RUN " : ""}baking Tier-2 node facets for ` +
        ...
        (skipRoads ? ", ROADS-GATED-OFF (FEMA-only)" : ", ROADS-ENABLED") +
        (skipFema ? ", SKIP-FEMA" : "") +
        (skipRoads
          ? `, fema-concurrency=${femaConcurrency}`
