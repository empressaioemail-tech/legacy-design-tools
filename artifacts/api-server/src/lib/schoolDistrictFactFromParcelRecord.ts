/**
 * parcel_record -> SchoolDistrictFactRead adapter (F-01, serve/prod
 * cutover, `_decisions/2026-09-02_step7_consumer_c_then_b.md`).
 *
 * schoolDistrict is a scalar cell with NO companion rows -- its extra
 * fields (`districtCode`, `geoid`) live on the cell_state itself
 ...
 * an absence -- it becomes a distinct refusal code. See
 * schoolDistrictFactRead.ts's module doc for the known zero-hit/multi-hit
 * anomaly class this rail never writes absent-verified for.
 */

import { loadParcelRecordCell } from "./parcelRecordCellRead";
import { parseParcelNodeId } from "./parcelNodeId";
import {
  SCHOOL_DISTRICT_FACT_SOURCE,
...
export async function schoolDistrictFactFromParcelRecord(
  parcelNodeId: string,
  ...
  const cell = await loadParcelRecordCell(parsed.countyFips, parsed.propId, SCHOOL_DISTRICT_RAIL_KEY);
  ...
  // cell.state === "present"
  const districtName = asNullableString(cell.value);
  if (!districtName) {
    return {
      state: "refused",
      ...
      source: SCHOOL_DISTRICT_FACT_SOURCE,
      entityId: placeKey,
      reason: `parcel_record_cell ${placeKey}/${SCHOOL_DISTRICT_RAIL_KEY} is kind=value but its value is not a readable district name. Refusing rather than inventing one.`,
    };
  }
  ...
  return {
    ...
    geoid: asNullableString(cell.raw.geoid),
    sourceAdapter: "parcel_record",
