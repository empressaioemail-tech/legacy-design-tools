...
const WILLIAMSON = "48491:R391328"; // real program county, never slated for this rail.
...
describe("loadMaxFootprintSqFtFactForServe — no county is slated for this rail yet", () => {
  it("a real program county resolves to the typed not-cut-over refusal, never a store call", async () => {
    ...
    const result = await loadMaxFootprintSqFtFactForServe(WILLIAMSON);
  ...
  it("FALSIFIER: even a fabricated PASS verdict has no effect — the slate check short-circuits first", async () => {
    setMaxFootprintSqFtVerdictStoreForTests(
      memoryParcelGateVerdicts([
        { countyFips: "48491", railKey: "maxFootprintSqFt", verdict: "pass", unaccountedCount: 0, evaluatedAt: "2026-09-11T00:00:00Z", runId: "test" },
      ]),
    );
    const result = await loadMaxFootprintSqFtFactForServe(WILLIAMSON);
