/**
 * schoolDistrictFactServeCutover.ts — P-297 (operator ruling A-193, OPS-24 law 7): the serve switch
 * is the code-owned SLATE, and what a parcel shows comes from ITS OWN cell.
 *
 * Rail note: schoolDistrict has no legacy reader; its fallback is a not-cut-over refusal.
 *
...
import { loadSchoolDistrictFactForServe } from "./schoolDistrictFactServeCutover";
import {
...
import { notCutOverSchoolDistrictFact } from "./schoolDistrictFactRead";

const RAIL_KEY = "schoolDistrict";
...
describe("schoolDistrictFactServeCutover — the slate is the switch", () => {
  ...
  it("an UNSLATED pair never touches parcel_record — it runs the pre-cutover path, unchanged, with zero I/O", async () => {
    const withoutStore = await loadSchoolDistrictFactForServe(UNSLATED);
    const counter = countingStore();
    setParcelRecordQueryableForTests(counter.store);
    const withStore = await loadSchoolDistrictFactForServe(UNSLATED);
  ...
  it("FALSIFIER 2: an unaccounted cell on a SLATED pair is a declared refusal carrying the cell's reason, never the pre-cutover answer", async () => {
    ...
    const served = await loadSchoolDistrictFactForServe(SLATED);
    const preCutover = notCutOverSchoolDistrictFact(SLATED);
  ...
  it("a malformed parcelNodeId keeps the pre-cutover path's own answer (no place_key is guessed)", async () => {
    ...
    const viaWrapper = await loadSchoolDistrictFactForServe(malformed);
    resetParcelRecordQueryableForTests();
    const withoutStore = await loadSchoolDistrictFactForServe(malformed);
