/**
 * specialDistrictFactServeCutover.ts — P-297 (operator ruling A-193, OPS-24 law 7): the serve switch
 * is the code-owned SLATE, and what a parcel shows comes from ITS OWN cell.
 *
 * Rail note: specialDistricts' slate deliberately does NOT include Caldwell (48055), so
 * Caldwell keeps the atom path -- a slate decision, made once in code and visible, which is
 * exactly what the ruling wants a county-wide verdict to stop doing.
 *
...
import { loadSpecialDistrictFactForServe } from "./specialDistrictFactServeCutover";
import {
...
import { loadSpecialDistrictFactAtom } from "./specialDistrictFactRead";

const RAIL_KEY = "specialDistricts";
...
describe("specialDistrictFactServeCutover — the slate is the switch", () => {
  ...
  it("an UNSLATED pair never touches parcel_record — it runs the pre-cutover path, unchanged, with zero I/O", async () => {
    const withoutStore = await loadSpecialDistrictFactForServe(UNSLATED);
    const counter = countingStore();
    setParcelRecordQueryableForTests(counter.store);
    const withStore = await loadSpecialDistrictFactForServe(UNSLATED);
  ...
  it("FALSIFIER 2: an unaccounted cell on a SLATED pair is a declared refusal carrying the cell's reason, never the pre-cutover answer", async () => {
    ...
    const served = await loadSpecialDistrictFactForServe(SLATED);
    const preCutover = loadSpecialDistrictFactAtom(SLATED);
  ...
  it("a malformed parcelNodeId keeps the pre-cutover path's own answer (no place_key is guessed)", async () => {
    ...
    resetParcelRecordQueryableForTests();
    const withoutStore = await loadSpecialDistrictFactForServe(malformed);
