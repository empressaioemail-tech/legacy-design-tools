/**
 * parcel_record -> MaxHeightFtFactRead adapter (F-01, OPS-21 P-148 / P-133),
 * mirroring maxImperviousCoverPctFactFromParcelRecord.ts's own shape for a
 * scalar cell with no companion rows -- its extra fields (`citationUrl`,
 * `districtCode`, `districtName`, `jurisdictionKey`) live on the cell_state
 * itself (parcel-envelope-cells.mjs's envelope cell_state), read via
...
export async function maxHeightFtFactFromParcelRecord(
  parcelNodeId: string,
  ...
  const cell = await loadParcelRecordCell(parsed.countyFips, parsed.propId, MAX_HEIGHT_FT_RAIL_KEY);
  ...
  // cell.state === "present"
  const feet = asNullableNumber(cell.value);
  if (feet === null) {
    return {
      state: "refused",
      ...
      source: MAX_HEIGHT_FT_FACT_SOURCE,
      entityId: placeKey,
      reason: `parcel_record_cell ${placeKey}/${MAX_HEIGHT_FT_RAIL_KEY} is kind=value but its value is not a readable number. Refusing rather than inventing one.`,
    };
  }
