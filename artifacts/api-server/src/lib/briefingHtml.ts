...
export interface RenderBriefingHtmlInput {
  ...
  /**
   * Footer watermark text for `@page @bottom-center`. When omitted,
   * uses {@link DEFAULT_FOOTER_WATERMARK}. Pass empty string to hide.
   */
  footerWatermark?: string;
  /** Cover page title accent color (CSS color). Defaults cockpit cyan. */
...
function renderHead(input: RenderBriefingHtmlInput, header: string): string {
  ...
  const footerWatermark =
    input.footerWatermark !== undefined
      ? input.footerWatermark
      : DEFAULT_FOOTER_WATERMARK;
  const coverAccent = input.coverAccentColor ?? "#00b4d8";
  return `<head>
<meta charset="utf-8" />
...
<style>
  @page {
    ...
    @bottom-center { content: "${esc(footerWatermark)}"; font-family: ${BRIEFING_PDF_FOOTER_TOKENS.fontFamily}; font-size: ${BRIEFING_PDF_FOOTER_TOKENS.fontSize}; color: ${BRIEFING_PDF_FOOTER_TOKENS.color}; }
  ...
  h1.briefing-cover-title { color: ${coverAccent}; }
...
function renderCover(input: RenderBriefingHtmlInput): string {
  ...
  <h1 class="briefing-cover-title" style="color: ${esc(coverAccent)}">Stakeholder Briefing</h1>
