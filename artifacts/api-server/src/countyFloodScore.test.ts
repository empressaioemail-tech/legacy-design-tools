/**
 * Flood-rail scorer — pure-core unit tests (no DB).
 *
 * Covers `scoreFlood`: given atom count + txgio_parcel DISTINCT
 ...
 * the Texas rollup).
 *
 * Atom keying note (preserve): stored flood atoms carry NULL
 * body.countyFips; the scorer keys on left(entity_id, 5).
 */

import { describe, it, expect } from "vitest";
import { scoreFlood } from "./countyFloodScoreCli";
...
describe("scoreFlood", () => {
  ...
  it("source is flood-hazard-fact-atom-count when features present, null when absent", () => {
    const present = scoreFlood({
      fips: "48261",
      name: "Kenedy",
      atomCount: 529,
      featureCount: 538,
    });
    expect(present.facet.source).toBe("flood-hazard-fact-atom-count");

    const absent = scoreFlood({
      fips: "48999",
      name: "NoSource",
      atomCount: 0,
      featureCount: null,
