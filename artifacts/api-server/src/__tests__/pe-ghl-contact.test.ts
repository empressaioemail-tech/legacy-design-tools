 ...
 *
 * Covers:
 *   - a new signup sends Explorer + ss_src_direct (no UTMs) and writes
 *     no retired `source-*` / `tier-*` tag
...
describe("session-exchange -> GHL contact on new signup", () => {
  ...
  it("a paid-ad campaign on session-exchange writes ss_src_ad and stores the UTMs", async () => {
    ...
    const res = await exchangeAuth(
      ...
      provider: "google",
      subject: "google-subject-ghl-ad",
      email: "ghl-ad@example.com",
      displayName: "Ad Signup",
    ...
    expect(values).toEqual(
      expect.arrayContaining(["free", "none", "facebook", "paid", "phase1", "v2"]),
    );
  });
  ...
  it("a returning user (isNewUser: false) never triggers a GHL contact call", async () => {
    ...
    // No further GHL call for the returning sign-in.
