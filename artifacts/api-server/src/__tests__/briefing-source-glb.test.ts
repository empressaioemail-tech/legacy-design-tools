...
describe("GET /api/briefing-sources/:id/glb", () => {
  it("is reachable without an internal audience (QA-30/31)", async () => {
    ...
    // on /briefing-sources/:id/glb — production fails every session
    // closed to audience:"user", which dead-locked the architect's own
    // Site Context 3D viewer. The route now reaches its handler
