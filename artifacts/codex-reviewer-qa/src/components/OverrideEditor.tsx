/**
 * Codex Reviewer QA — finding override (edit) form (CDX-4).
 *
 * The reviewer edits a finding's text / severity / category and gives a
 * required reason; submitting calls `POST /findings/{id}/override`,
 * which cortex-api persists as a single revision row carrying the
 * reviewer's attribution + timestamp. Pre-filled from the current
 * finding so an edit starts from the engine's own wording.
 */
...
export function OverrideEditor({
  ...
  const canSubmit =
    text.trim() !== "" && reviewerComment.trim() !== "" && busy !== true;
