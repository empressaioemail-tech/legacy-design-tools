/**
 * End-to-end CLI smoke test for the parcel_briefings.prior_generated_at
 * backfill script.
 *
 * Why this exists (Task #334)
 * ---------------------------
 * `backfillPriorGeneratedAt.test.ts` already covers `backfill()`
 * and `parseArgs()` as in-process functions, but the actual entrypoint
 * — what `pnpm --filter @workspace/scripts run backfill:prior-
 * generated-at` invokes via `tsx` — is glue code we can't reach
 * from a unit test:
 *
 ...
 *
 * Mirrors `backfillBriefingGenerationIds.cli.test.ts` (Task #315) —
 * same script-shape, same operator-facing log contract, so the smoke
 * tests are intentionally near-identical in structure.
 *
 * Strategy
 * --------
...
/**
 * Seed the same one-engagement / two-jobs / one-briefing fixture the
 * unit-test happy-path uses: a "prior" completed job that finished
 * before `generated_at` (the candidate the backfill must pick), and
 * a "current" completed job whose window contains `generated_at`
 * (which the strict `< pb.generated_at` guard must exclude). The
 * briefing carries `prior_generated_by` but no `prior_generated_at`
 * — exactly the legacy shape the backfill targets.
 *
 * Duplicated rather than shared so the smoke test stays
 * self-contained — if the unit-test seed shifts to cover a different
 * scenario, the smoke-test contract on the CLI's stdout shouldn't
 * drift with it.
 */
async function seedOneMatchableBriefing(ctx: TestSchemaContext): Promise<{
  ...
  // Prior job ran from t-100s to t-95s — strictly before the
  // briefing's generated_at (t-7s). The backfill must pick this
  // one's completed_at.
  ...
  // briefing's generated_at (t-7s). The strict `<` guard must
  // exclude it; if it ever loosens, the side-effect assertion in
  // the happy-path test catches the drift.
...
describe("backfillPriorGeneratedAt CLI — Task #334", () => {
  ...
  it("runs cleanly to completion against a seeded fixture", async () => {
    await withTestSchema(async (ctx) => {
      const { briefingId, priorJobCompletedAt } =
        await seedOneMatchableBriefing(ctx);
      ...
      expect(res.stdout).toContain(
        "Backfilling parcel_briefings.prior_generated_at (dryRun=false)",
      );
      ...
      // Side effect: the column actually got stamped, with the
      // prior job's completed_at (NOT the current job's). This
      // guards against a future refactor that prints a clean
      // summary while silently failing to mutate (e.g. forgetting
      // to await the UPDATE) and against a refactor that loosens
      // the `completed_at < pb.generated_at` guard and starts
      // picking the current job.
  ...
  it("prints the dry-run preview lines and stable summary without mutating", async () => {
    await withTestSchema(async (ctx) => {
      const { briefingId } = await seedOneMatchableBriefing(ctx);
      ...
      expect(res.stdout).toContain(
        "Backfilling parcel_briefings.prior_generated_at (dryRun=true)",
      );
      // Per-row preview line. Format:
      //   `[dry-run] briefing <uuid> → <iso-timestamp>`
      // We assert the briefing id and the arrow so a refactor that
      // swaps the arrow direction or drops the briefing id would
      // fail loudly. We don't pin the timestamp serialisation
      // because Postgres' default `timestamp` text format is
      // implementation-defined; the briefing id + arrow + a non-
      // "no matching" tail is enough to prove the matched branch
      // was taken.
      expect(res.stdout).toContain(`[dry-run] briefing ${briefingId} → `);
      expect(res.stdout).not.toContain(
        `[dry-run] briefing ${briefingId} → no matching prior job`,
      );
