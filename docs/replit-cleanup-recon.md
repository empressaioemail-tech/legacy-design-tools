...
Branches           : 311 total — 1 active (main), 1 backup (gitsafe-backup/main),
                     1 Replit auto-track (replit-agent), 306 subrepl-* leftovers
...
| 3 | **306 `subrepl-*` git branches** | Local prune (`git branch -D` + `git remote remove`) | These are the Replit Agent's per-task isolation branches — every Replit task that ran in an isolated subrepl left one behind. They are dated 2026-04-30 to 2026-05-01 (the entire active sprint window). Their work has either landed on `main` or been cancelled. They are not used for rollback (the `gitsafe-backup/main` remote handles that). They also exist as named git remotes (`subrepl-XXXX` for each), polluting `git remote -v`. **Caveat:** confirm with Replit support that pruning subrepl branches doesn't break the platform's task-history view; the workspace-cleanup-purge sprint should test on one branch first before doing all 306. |
...
| `gitsafe-backup/main` remote | The named "backup/2026-04-29-replit-local-main" preserve-target. Last commit `a74a5fc` 2026-04-29 17:19 UTC ("Update deferred tasks list with EP-003 details"). This is the rollback safety net — keep until Cursor/Claude Code is established. |
...
4. **`gitsafe-backup/main` is 3 days behind main.** Last commit on the backup remote is 2026-04-29 17:19 UTC; main has had 30+ commits since (entire Wave 2 substrate + reviewer-parity + AIR-2 cluster). If this remote is the documented rollback target, the rollback window is "Wave 1 closeout" — anything reviewer-parity / AIR / Wave 2 would be lost in a rollback. **Worth a manual `git push gitsafe-backup main` to advance the backup before the cleanup sprint touches anything.**
5. **`SESSION_SECRET` is set as a secret but I cannot find a single grep hit for it in code.** This is one of two outcomes: (a) it's read by a session middleware via a generic `process.env.SESSION_SECRET` access pattern that my regex missed, or (b) it's actually unused and someone set it during initial setup and forgot. Worth a 2-minute confirmation before declaring it stale.
6. **`OPENAI_API_KEY` is set but only used for embeddings (`lib/codes/src/embeddings.ts`).** The codebase already gracefully handles `OPENAI_API_KEY` being unset (`embeddings.ts:37` checks for it; `:45` logs the fallback). If embeddings are no longer being generated against OpenAI, this secret is removable. **Cross-check:** is the codes corpus still being re-ingested, or is the corpus frozen post-DA-PI-2/4? If frozen, `OPENAI_API_KEY` can go.
7. **The `[deployment.postBuild]` runs `pnpm store prune`** — this is correct and load-bearing, just flagging because if anyone tries to "speed up the deploy" by removing it, deployed image size will balloon.
8. **No `AGENTS.md`, `CLAUDE.md`, `.cursorrules`, `.cursor/`, or `.claude/` exists.** Cursor / Claude Code transition will need at least one of these created from scratch — `replit.md`'s User Preferences section is the best starting material.
