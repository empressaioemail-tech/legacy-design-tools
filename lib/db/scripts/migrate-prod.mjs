...
// Surface RAISE NOTICE output from inside applied migrations (e.g. 0097's
// orphan-cleanup row counts) -- pg's Client silently drops NOTICE messages
// with no listener attached, so without this a migration's own accounting
// of what it did never reaches the run-migrations log.
