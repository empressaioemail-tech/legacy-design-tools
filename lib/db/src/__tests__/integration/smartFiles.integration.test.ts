...
const ENTITY_ID = "smartfile:48021:udc-2024";
...
async function seedDocument(
  ...
  const { rows } = await pool.query(
    `INSERT INTO "${schemaName}".smart_file_documents
       (entity_id, jurisdiction_fips, doc_slug, title, access_policy, current_version)
     VALUES ($1, '48021', 'udc-2024', 'Unified Development Code', 'public-free', 1)
     RETURNING id`,
    [entityId],
  );
      ...
      for (const [targetType, targetId] of [
        ...
        await pool.query(
          `INSERT INTO "${schemaName}".smart_file_placements
           ...
           VALUES ($1, $2, $3, $4)`,
          [docId, ENTITY_ID, targetType, targetId],
        );
      }
  ...
  it("refuses a SECOND document row for the same declared entityId", async () => {
    ...
    await withTestSchema(async ({ pool, schemaName }) => {
      ...
      try {
        await pool.query(
          `INSERT INTO "${schemaName}".smart_file_documents
             (entity_id, jurisdiction_fips, doc_slug, title, access_policy)
           VALUES ($1, '48021', 'udc-2024', 'Duplicate', 'public-free')`,
          [ENTITY_ID],
        );
      } catch (e) {
    ...
    await withTestSchema(async ({ pool, schemaName }) => {
      ...
      try {
        await pool.query(
          `INSERT INTO "${schemaName}".smart_file_documents
             (entity_id, jurisdiction_fips, doc_slug, title, access_policy)
           VALUES ('smartfile:48021:bad-policy', '48021', 'bad-policy', 'X', 'public')`,
        );
      } catch (e) {
        code = (e as { code?: string }).code;
      }
